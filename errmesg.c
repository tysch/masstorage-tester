...
void printwriteerr(FILE * logfile, int islogging)
{
	...
	switch (errno)
	{
        ...
        case EINTR:
            ...
            break;
        case EIO:
            errstr = "\nA physical I/O error has occurred.The process is a member of a background process group attempting to write to its controlling terminal, TOSTOP is set, the calling thread is not blocking SIGTTOU, the process is not ignoring SIGTTOU, and the process group of the process is orphaned. This error may also be returned under implementation-defined conditions.\n";
            break;
        case ENOSPC:
        ...
        case EPIPE:
            ...
            break;
        case EINVAL:
            errstr = "\nThe STREAM or multiplexer referenced by fildes is linked (directly or indirectly) downstream from a multiplexer.The file is a regular file or block special file, and the offset argument is negative. The file offset shall remain unchanged\n";
            break;
        case ENOBUFS:
            errstr = "\nInsufficient resources were available in the system to perform the operation.\n";
            break;
        case ENXIO:
            errstr = "\nA request was made of a nonexistent device, or the request was outside the capabilities of the device.A hangup occurred on the STREAM being writtenA write to a STREAMS file may fail if an error message has been received at the STREAM head. In this case, errno is set to the value included in the error message.\n";
            break;
        case EACCES:
